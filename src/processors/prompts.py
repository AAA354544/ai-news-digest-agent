...
def build_digest_system_prompt() -> str:
    return (
        ...
        '每个 item 必须包含 title, links, tags, summary, mechanism, why_it_matters, insights, source_names。'
        'links 必须来自候选 URL。'
        '不得生成候选中不存在的新链接。'
        '正文标题允许保留英文原题，但 title 字段必须采用“英文原题｜中文副标题”格式；如果原题已是中文，可只写中文标题。'
        'summary、mechanism、why_it_matters、insights 必须全部使用中文，不得输出英文段落。'
        'summary 需要信息密度充足，建议 3-5 句中文或约 120-220 个中文字符；不要只翻译标题或只写一句短摘要。'
        'summary 必须说明事件/论文/项目/产品更新是什么，并标明候选材料能确认的信息边界。'
        'mechanism 建议 1-3 句中文，说明背后的机制、技术路径、商业逻辑、生态传播机制或影响链条；无法确定时写保守解释，不要编造。'
...
def build_digest_user_prompt(
    ...
    return (
        ...
        '正文语言规则：summary、mechanism、why_it_matters、insights、brief_summary 必须使用中文；不要输出英文摘要或英文 why it matters。\n'
        '摘要长度规则：summary 建议 3-5 句中文或约 120-220 个中文字符；必须说明事件本身、已知信息边界，不要空泛。\n'
        '机制说明规则：mechanism 建议 1-3 句中文，解释技术路径、商业逻辑、生态传播机制或影响链条；不能确定时用“候选信息显示/可能/尚未明确”等保守措辞。\n'
